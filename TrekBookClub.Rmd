...
ggplot(trek.dsc, aes(x = year_month, y = n_votes, label = dsc)) +
  ...
  geom_text(aes(label=dsc),hjust=1.15, vjust=-.25) +
  scale_fill_manual(values = cb_palette) +
  theme_light() + 
...
1. Try to cleanly infer the raw numbers of voters who gave each score based on the proportion. It's mostly there, but I hope I can get it to add up to one for real!
...
There's a negative correlation with C grades, but I'll have to figure out some statistical test to see if that's real or not. There are very few books that have gottn over 50 votes, so the small proportion of Cs in the three most highly voted for novels may be dragging the lm down on the right
```{r}
